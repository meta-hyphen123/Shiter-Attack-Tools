...
path = input('Inject payloads in the path? (y/n): ').lower() == 'y'
jsonData = input('Treat post data as JSON? (y/n): ').lower() == 'y'
paramData = input('Enter post data: ') if not jsonData else None
encode = input('Encode payloads? (base64/plain): ').lower()
fuzz = input('Use fuzzer? (y/n): ').lower() == 'y'
update = input('Update? (y/n): ').lower() == 'y'
timeout = int(input('Enter timeout: '))
proxy = input('Use proxy? (y/n): ').lower() == 'y'
recursive = input('Crawl? (y/n): ').lower() == 'y'
args_seeds = input('Load crawling seeds from a file? (Enter file path or press Enter): ')
args_file = input('Load payloads from a file? (Enter file path or press Enter): ')
level = int(input('Enter level of crawling: '))
add_headers = input('Add headers? (Enter headers or press Enter): ')
threadCount = int(input('Enter number of threads: '))
delay = int(input('Enter delay between requests: '))
skip = input('Skip asking to continue? (y/n): ').lower() == 'y'
skipDOM = input('Skip DOM checking? (y/n): ').lower() == 'y'
blindXSS = input('Inject blind XSS payload while crawling? (y/n): ').lower() == 'y'
